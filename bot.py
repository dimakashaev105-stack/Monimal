...
def init_db():
    with db() as c:
        ...
        c.execute("INSERT INTO lottery (id, jackpot, draw_at) VALUES (1, 0, 0) ON CONFLICT DO NOTHING")
        c.execute("""
...
def cmd_start(msg):
    ...
    bot.send_message(uid, f"🌸 <b>Добро пожаловать, {name}!</b>\n\nВыбери раздел в меню 👇", reply_markup=main_menu(), parse_mode="HTML")
...
def cmd_taxi(msg):
    ...
    bot.send_message(uid,
        f"🚕 <b>Новый заказ!</b>\n\nМаршрут: {route['name']}\nВремя: {route['time']} мин\n"
        f"Оплата: <b>{fmt(earn)} {CURRENCY}</b>\n\n✅ Поехали? Напиши <b>завершить</b> через {route['time']} мин",
        parse_mode="HTML")
    def finish():
...
def cmd_darts(msg):
    ...
    result = random.randint(1, 6)
    if result == 6:
        update_balance(uid, bet * 5 - bet)
        text = f"🎯 <b>ЯБЛОЧКО!</b> +{fmt(bet * 4)} {CURRENCY}"
    elif result in [4, 5]:
...
def cmd_bowling(msg):
    ...
    result = random.randint(1, 6)
    if result == 6:
        update_balance(uid, bet * 3 - bet)
        text = f"🎳 <b>СТРАЙК!</b> x3! +{fmt(bet * 2)} {CURRENCY}"
    elif result == 5:
        win = int(bet * 1.5)
        update_balance(uid, win - bet)
...
def cmd_football(msg):
    ...
    if result in [3, 4]:
        update_balance(uid, bet)
        text = f"⚽ <b>ГОЛ!</b> x2! +{fmt(bet)} {CURRENCY}"
    else:
        update_balance(uid, -bet)
...
def cmd_mines(msg):
    ...
    mines_games[uid] = {"bet": bet, "mines": mine_positions, "opened": []}
    update_balance(uid, -bet)
...
def cmd_repay(msg):
    ...
    with db() as c:
        if new_debt <= 0:
            ...
            bot.send_message(uid, f"✅ Кредит погашен полностью!", parse_mode="HTML")
        else:
...
def cmd_buy_stock(msg):
    ...
    u = get_user(uid)
    with db() as c:
        c.execute("SELECT last_trade FROM stock_portfolio WHERE user_id=%s AND ticker=%s", (uid, TICKER))
        row = c.fetchone()
    ...
    with db() as c:
        ...
        port = c.fetchone()
        if port and int(port["shares"]) + qty > STOCK_MAX_PER_USER:
        ...
        c.execute("INSERT INTO stock_trades (user_id, ticker, action, amount, price, total, created_at) VALUES (%s,%s,%s,%s,%s,%s,%s)",
...
def cmd_sell_stock(msg):
    ...
    with db() as c:
        ...
        if not port or int(port["shares"]) < qty:
            bot.send_message(uid, f"❌ У тебя {int(port['shares']) if port else 0} акций", parse_mode="HTML")
            return
        ...
        if new_shares == 0:
            ...
            c.execute("UPDATE stock_portfolio SET shares=%s, last_trade=%s WHERE user_id=%s AND ticker=%s",
                      (new_shares, now(), uid, TICKER))
        c.execute("INSERT INTO stock_trades (user_id, ticker, action, amount, price, fee, total, created_at) VALUES (%s,%s,%s,%s,%s,%s,%s,%s)",
...
def cmd_stock_history(msg):
    ...
    with db() as c:
        c.execute("SELECT action, amount, price, created_at FROM stock_trades WHERE user_id=%s ORDER BY created_at DESC LIMIT 5", (uid,))
        trades = c.fetchall()
    trade_lines = []
    for row in trades:
...
def cmd_top(msg):
    ...
    for i, row in enumerate(rows):
        text += f"{medals[i]} {row['name'] or 'Игрок'} — <b>{fmt(row['balance'])}</b>\n"
    with db() as c:
...
def cmd_transfer(msg):
    ...
    if int(u["balance"]) < total:
        ...
        return
    if now() - int(u.get("last_transfer") or 0) < CD_TRANSFER:
...
def cmd_promo(msg):
    ...
    if not promo:
        ...
        return
    if promo["expires"] and int(promo["expires"]) < now():
...
def cmd_check(msg):
    ...
    update_balance(uid, -total)
    with db() as c:
        c.execute("INSERT INTO checks (code, amount, max_activations, created_by, created_at) VALUES (%s,%s,%s,%s,%s)",
...
def cmd_activate_check(msg):
    ...
    with db() as c:
        try:
            c.execute("INSERT INTO check_activations (user_id, check_code, activated_at) VALUES (%s,%s,%s)",
                      (uid, code, now()))
        except:
...
def cmd_give(msg):
    ...
    with db() as c:
        if target.isdigit():
            c.execute("SELECT id FROM users WHERE id=%s", (int(target),))
        else:
            c.execute("SELECT id FROM users WHERE name ILIKE %s", (f"%{target}%",))
        row = c.fetchone()
        if not row:
...
def cmd_active(msg):
    ...
    pct = daily / total * 100 if total else 0
    bot.send_message(msg.chat.id, f"📊 Активность за сутки: {daily}/{total} ({pct:.1f}%)")
...
if __name__ == "__main__":
    ...
    print("🚀 Бот запущен (PostgreSQL/Supabase)")
