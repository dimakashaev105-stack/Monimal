...
def _apply_trade_impact(qty, is_buy):
    """
    Линейный impact без капа — сколько купил/продал, столько и двинулась цена.
    1 акция = STOCK_BUY_IMPACT (0.003%).
    100 акций = 0.3%, 500 = 1.5%, 1000 = 3%, 5000 = 15%.
    """
    price, _ = _get_stock_price()
    impact = STOCK_BUY_IMPACT * qty if is_buy else -STOCK_SELL_IMPACT * qty
    new_price = _set_stock_price(price * (1 + impact))
    return new_price
